...
class ExcelReporter {
    ...
    async generate(outputPath) {
        ...
        titleCell.value = 'Smart Press Web E2E Automation Test Summary';
        titleCell.font = { name: 'Segoe UI', size: 16, bold: true, color: { argb: 'FFFFFF' } };
        titleCell.alignment = { vertical: 'middle', horizontal: 'center' };
        titleCell.fill = {
            type: 'pattern',
            pattern: 'solid',
            fgColor: { argb: '0F766E' } // Sleek Dark Teal color for Web branding
        };
        ...
        dataRows.forEach(item => {
            ...
            if (item.metric === 'Overall Pass Rate') {
                const color = failedSteps > 0 ? '991B1B' : '065F46';
        ...
        this.steps.forEach((step) => {
            ...
            if (step.status === 'PASS') {
                statusCell.fill = {
                    type: 'pattern',
                    pattern: 'solid',
                    fgColor: { argb: 'D1FAE5' } // Light Emerald
                };
                statusCell.font = { name: 'Segoe UI', size: 10, bold: true, color: { argb: '065F46' } };
            } else {
                statusCell.fill = {
                    type: 'pattern',
                    pattern: 'solid',
                    fgColor: { argb: 'FEE2E2' } // Light Rose
                };
