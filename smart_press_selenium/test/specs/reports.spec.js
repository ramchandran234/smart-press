describe('Smart Press Web - Reports & Export E2E Flow', () => {
    ...
    it('should navigate to export screen and select formats', async () => {
        ...
        await exportTile.waitForDisplayed({ timeout: 10000 });
        ...
        await excelBtn.waitForDisplayed({ timeout: 10000 });
        await excelBtn.click();
    });

    it('should run export action and verify download start toast', async () => {
        console.log('Step 6: Click "Export Excel" action button');
        const exportActionBtn = await $('//*[contains(text(), "Export Excel")] | //flt-semantics[contains(@aria-label, "Export Excel")]');
        await exportActionBtn.waitForDisplayed({ timeout: 10000 });
